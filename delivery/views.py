...
def view_menu(request, restaurant_id, username=None):
...
@csrf_exempt
def add_to_cart(request, item_id, username=None):
    if request.method == 'POST':
        if 'username' not in request.session:
...
def show_cart(request, username=None):
    if 'username' not in request.session:
        return redirect('open_signin')
...
# Orders Page
def orders(request, username=None):
    if 'username' not in request.session:
        return redirect('open_signin')
